...
def agent_2_3_logic(state: PipelineState) -> PipelineState:
    ...
    위 상황을 바탕으로 클릭률(CTR)을 높이기 위한 실험 가설 3가지를 세우고, 
    그 중 가장 타당한 1가지를 선택하여 그 이유를 설명하세요.
    """
...
def agent_4_final_report(state: PipelineState) -> PipelineState:
    ...
    prompt = f"""
    당신은 시니어 퍼포먼스 마케터입니다. 아래 가설을 바탕으로 디자이너에게 전달할 'B안 제작 기획서'를 작성하세요.
    
    [가설 및 전략]: {hypo_info}
    
    포함될 내용:
    1. 실험명 및 KPI 목표
    2. B안 수정 포인트 (텍스트, 배치, 버튼 등 구체적 지시)
    3. 디자인 가이드라인 (색상, 폰트 강조점 등)
    4. 기대 효과
    """
...
if __name__ == "__main__":
    ...
    target_img = os.path.join(current_dir, "..", "data", "a안.png")
    ...
    initial_state = {
        ...
        "scenario": """당신은 뷰티 커머스 '올리브영' 앱의 마케터입니다. 
        GA4 데이터를 분석한 결과, '이벤트/기획전 페이지'의 트래픽은 매우 높지만, 
        해당 페이지에서 '상품 상세 페이지'로 넘어가는 클릭률(CTR)이 현저히 낮습니다. 
        고객들이 이벤트 광고를 보고 들어오긴 하는데, 정작 이벤트 상품을 클릭하지 않고 그냥 나가버립니다."""
    }
